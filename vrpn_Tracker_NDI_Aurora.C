...
#include "vrpn_Tracker_NDI_Aurora.h"
#include "vrpn_Types.h"                 // for vrpn_float64

vrpn_Tracker_NDI_Aurora::vrpn_Tracker_NDI_Aurora(const char *name,
	vrpn_Connection *c,
	const char *port,
	...
	const char** rigidBodyNDIRomFileNames) : vrpn_Tracker(name, c)
{
	...
	if (serialFd == -1) {
		fprintf(stderr, "vrpn_Tracker_NDI_Aurora: Can't open serial port: %s\n", port);
	}
	else {

		printf("connected to NDI Aurora at default 9600 baud on device:%s.\n", port);
...
//#endif
		...
		sendCommand("PENA 0AD");
		readResponse();
		
		sendCommand("PENA 0BD");
		readResponse();

		sendCommand("PINIT 0A");
		readResponse();

		sendCommand("PINIT 0B");
		readResponse();
...
vrpn_Tracker_NDI_Aurora::~vrpn_Tracker_NDI_Aurora()
{
	vrpn_close_commport(serialFd);
}

void vrpn_Tracker_NDI_Aurora::mainloop()
{
	get_report();
...
int vrpn_Tracker_NDI_Aurora::get_report(void)
// returns 0 on fail
{
...
void vrpn_Tracker_NDI_Aurora::send_report(void) // called from get_report()
{
	if (d_connection)
	{
...
void vrpn_Tracker_NDI_Aurora::sendCommand(const char* commandString) {
...
int vrpn_Tracker_NDI_Aurora::readResponse() {
...
// , or -1 on failure
int vrpn_Tracker_NDI_Aurora::convertBinaryFileToAsciiEncodedHex(const char* filename, char *asciiEncodedHexStr)
{
...
unsigned int vrpn_Tracker_NDI_Aurora::parse2CharIntFromNDIResponse(unsigned char* str, int* strIndexPtr) {
	unsigned intVal;
...
float vrpn_Tracker_NDI_Aurora::parse6CharFloatFromNDIResponse(unsigned char* str, int* strIndexPtr) {
	int intVal;
...
float vrpn_Tracker_NDI_Aurora::parse7CharFloatFromNDIResponse(unsigned char* str, int* strIndexPtr) {
	int intVal;
...
int vrpn_Tracker_NDI_Aurora::setupOneTool(const char* NDIToolRomFilename)
{
...
void vrpn_Tracker_NDI_Aurora::switchToHigherBaudRate(const char *port) {
	printf("vrpn_Tracker_NDI_Polaris: Switching NDI to higher baud rate, and then reopening com port at higher rate...");
	sendCommand("COMM 60000"); // 1.2Mbit (1228739 baud), which requires the NDI USB 2.0 adapter
	readResponse();
#ifdef DEBUG
	...
	if (strncmp("RESET", (char *)latestResponseStr, 5) == 0) {
		...
		sendCommand("COMM 60000"); // 1.2Mbit, which requires the NDI USB 2.0 adapter
		readResponse();
		//printf("DEBUG: COMM response: >%s<\n",latestResponseStr);
	}
